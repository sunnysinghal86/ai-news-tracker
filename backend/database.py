...
Simple, zero-cost, zero-ops for small scale (10-20 users)
"""
...
DB_PATH = os.getenv("DB_PATH", "./data/news_tracker.db")
...
class User:
    ...
    name: str
    active: bool = True
    categories: List[str] = None  # filter preferences
    min_relevance: int = 5  # 1-10

    def to_dict(self):
        return {
            "id": self.id, "email": self.email, "name": self.name,
            "active": self.active, "categories": self.categories or [],
...
class Database:
    ...
    async def upsert_articles(self, articles: list):
        ...
        for a in articles:
            ...
            """, (
                ...
                a.published_at if isinstance(a.published_at, str) else a.published_at.isoformat(),
    ...
    async def get_articles(
        ...
        async with self._db.execute(
            f"SELECT * FROM articles WHERE {where} ORDER BY relevance_score DESC, fetched_at DESC LIMIT ? OFFSET ?",
            params
        ) as cursor:
    ...
    async def get_top_articles(self, limit=10, min_relevance=6) -> List[dict]:
        async with self._db.execute(
            "SELECT * FROM articles WHERE relevance_score >= ? AND emailed=0 ORDER BY relevance_score DESC, score DESC LIMIT ?",
            (min_relevance, limit)
        ) as cursor:
            rows = await cursor.fetchall()
        return [self._row_to_dict(row) for row in rows]
    
    async def mark_emailed(self, article_ids: List[str]):
        placeholders = ",".join("?" * len(article_ids))
        await self._db.execute(
            f"UPDATE articles SET emailed=1 WHERE id IN ({placeholders})", article_ids
        )
        await self._db.commit()
    ...
    async def get_stats(self) -> dict:
        ...
        async with self._db.execute(
            "SELECT COUNT(*) as products FROM articles WHERE is_product_or_tool=1"
        ) as c:
            products = (await c.fetchone())["products"]
        async with self._db.execute(
        ...
        return {
            ...
            "by_category": {row["category"]: row["count"] for row in cats}
        }
    
    # ── Users ─────────────────────────────────────
    async def create_user(self, email: str, name: str, categories=None, min_relevance=5) -> User:
        now = datetime.now(timezone.utc).isoformat()
        await self._db.execute(
            "INSERT OR IGNORE INTO users (email, name, active, categories, min_relevance, created_at) VALUES (?,?,1,?,?,?)",
    ...
    async def get_user_by_email(self, email: str) -> Optional[User]:
        async with self._db.execute("SELECT * FROM users WHERE email=?", (email,)) as c:
    ...
    async def log_digest(self, email: str, article_count: int, status: str):
        now = datetime.now(timezone.utc).isoformat()
        await self._db.execute(
            "INSERT INTO digest_log (sent_at, recipient_email, article_count, status) VALUES (?,?,?,?)",
